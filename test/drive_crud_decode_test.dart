...
void main() {
  group('GoogleDriveCrudClient.decodeBackupBytes', () {
    test(
      'new UTF-8 backup with em-dash, emoji and Danish letters round-trips',
      () {
        const content =
            '{"note":"recovery — day 1 🙏","navn":"Søren æble blå"}';
    ...
    test(
      'legacy Latin1 backup with Danish letters is preserved via fallback',
      () {
        const content = '{"navn":"Søren æble blå"}';
        final bytes =
            content.codeUnits; // legacy: each <= 0xFF char is one byte
