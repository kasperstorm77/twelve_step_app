...
void main() async {
  ...
  Future<T> timed<T>(String label, Future<T> Function() action) async {
    ...
    if (kDebugMode) {
      print(
        'startup: $label ${sw.elapsedMilliseconds}ms (total ${startupTotal.elapsedMilliseconds}ms)',
      );
    }
    return result;
  }
  ...
  try {
    await timed(
      "Hive.openBox('i_am_definitions')",
      () => Hive.openBox<IAmDefinition>('i_am_definitions'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening i_am_definitions box: $e');
    await timed(
      "Hive.deleteBoxFromDisk('i_am_definitions')",
      () => Hive.deleteBoxFromDisk('i_am_definitions'),
    );
    await timed(
      "Hive.openBox('i_am_definitions') [recreate]",
      () => Hive.openBox<IAmDefinition>('i_am_definitions'),
    );
    if (kDebugMode) {
  ...
  try {
    await timed(
      "Hive.openBox('people_box')",
      () => Hive.openBox<Person>('people_box'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening people_box: $e');
    await timed(
      "Hive.deleteBoxFromDisk('people_box')",
      () => Hive.deleteBoxFromDisk('people_box'),
    );
    await timed(
      "Hive.openBox('people_box') [recreate]",
      () => Hive.openBox<Person>('people_box'),
    );
  ...
  try {
    await timed(
      "Hive.openBox('reflections_box')",
      () => Hive.openBox<ReflectionEntry>('reflections_box'),
    );
  } catch (e) {
    ...
    try {
      await timed(
        "Hive.deleteBoxFromDisk('reflections_box')",
        () => Hive.deleteBoxFromDisk('reflections_box'),
      );
    } catch (deleteError) {
      if (kDebugMode) {
        print('Error deleting reflections_box: $deleteError (this is okay)');
      }
    ...
    await timed(
      "Hive.openBox('reflections_box') [recreate]",
      () => Hive.openBox<ReflectionEntry>('reflections_box'),
    );
    if (kDebugMode) {
      print('Cleared corrupted reflections_box and created new one');
    }
  }
  ...
  try {
    await timed(
      "Hive.openBox('gratitude_box')",
      () => Hive.openBox<GratitudeEntry>('gratitude_box'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening gratitude_box: $e');
    await timed(
      "Hive.deleteBoxFromDisk('gratitude_box')",
      () => Hive.deleteBoxFromDisk('gratitude_box'),
    );
    await timed(
      "Hive.openBox('gratitude_box') [recreate]",
      () => Hive.openBox<GratitudeEntry>('gratitude_box'),
    );
    if (kDebugMode) {
      print('Cleared corrupted gratitude_box and created new one');
    }
  }
  ...
  try {
    await timed(
      "Hive.openBox('agnosticism_pairs')",
      () => Hive.openBox<BarrierPowerPair>('agnosticism_pairs'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening agnosticism_pairs: $e');
    await timed(
      "Hive.deleteBoxFromDisk('agnosticism_pairs')",
      () => Hive.deleteBoxFromDisk('agnosticism_pairs'),
    );
    await timed(
      "Hive.openBox('agnosticism_pairs') [recreate]",
      () => Hive.openBox<BarrierPowerPair>('agnosticism_pairs'),
    );
    if (kDebugMode) {
      print('Cleared corrupted agnosticism_pairs and created new one');
    }
  }
  ...
  try {
    await timed(
      "Hive.openBox('morning_ritual_items')",
      () => Hive.openBox<RitualItem>('morning_ritual_items'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening morning_ritual_items: $e');
    await timed(
      "Hive.deleteBoxFromDisk('morning_ritual_items')",
      () => Hive.deleteBoxFromDisk('morning_ritual_items'),
    );
    await timed(
      "Hive.openBox('morning_ritual_items') [recreate]",
      () => Hive.openBox<RitualItem>('morning_ritual_items'),
    );
    if (kDebugMode) {
      print('Cleared corrupted morning_ritual_items and created new one');
    }
  }
  ...
  try {
    await timed(
      "Hive.openBox('morning_ritual_entries')",
      () => Hive.openBox<MorningRitualEntry>('morning_ritual_entries'),
    );
  } catch (e) {
    if (kDebugMode) print('Error opening morning_ritual_entries: $e');
    await timed(
      "Hive.deleteBoxFromDisk('morning_ritual_entries')",
      () => Hive.deleteBoxFromDisk('morning_ritual_entries'),
    );
    await timed(
      "Hive.openBox('morning_ritual_entries') [recreate]",
      () => Hive.openBox<MorningRitualEntry>('morning_ritual_entries'),
    );
    if (kDebugMode) {
      print('Cleared corrupted morning_ritual_entries and created new one');
    }
  }
  ...
  try {
    ...
    if (kDebugMode) {
      print('Cleared corrupted notifications_box and created new one');
    }
  }
  ...
  if (PlatformHelper.isMobile ||
    ...
    try {
      await timed(
        'AllAppsDriveService.initialize',
        () => AllAppsDriveService.instance.initialize(),
      );
      ...
      if (AllAppsDriveService.instance.isAuthenticated) {
        ...
        final enabled =
            settingsBox.get('syncEnabled', defaultValue: true) ?? true;
        await timed(
          "settings.put('syncEnabled')",
          () => settingsBox.put('syncEnabled', enabled),
        );
        await timed(
          'AllAppsDriveService.setSyncEnabled',
          () => AllAppsDriveService.instance.setSyncEnabled(enabled),
        );

        if (enabled) {
          final remoteNewer = await timed(
            'AllAppsDriveService.isRemoteNewer',
            () => AllAppsDriveService.instance.isRemoteNewer(),
          );
          if (remoteNewer) {
            if (kDebugMode) {
              print(
                'startup: ⚠️ Remote has newer data - blocking uploads until user fetches or dismisses',
              );
            }
        ...
        if (kDebugMode) {
          print(
            'startup: Drive not authenticated - sign in required in Data Management',
          );
        }
      }
    } catch (e) {
    ...
    if (kDebugMode) {
      print(
        'startup: Google Drive sync not available on ${PlatformHelper.platformName}',
      );
    }
  }
  ...
  if (AppSettingsService.shouldForceMorningRitual()) {
    if (kDebugMode) {
      print(
        'main: Within morning ritual window (first time today), setting morning ritual as selected app',
      );
    }
    await timed(
      'AppSwitcherService.setSelectedAppId(morningRitual)',
      () => AppSwitcherService.setSelectedAppId(AvailableApps.morningRitual),
    );
    await timed(
      'AppSettingsService.markMorningRitualForced',
      () => AppSettingsService.markMorningRitualForced(),
    );
  }
  ...
  startupTotal.stop();
  if (kDebugMode) {
    print('startup: runApp (total ${startupTotal.elapsedMilliseconds}ms)');
  }
